...
1. Route the PCBs in [`kicad/`](./v4/kicad/), and then save them to [`kicad/routed/`](./v4/kicad/routed/)
   * KiCad has no built-in autorouter. To autoroute with [Freerouting](https://github.com/freerouting/freerouting): File > Export > Specctra DSN, route the `.dsn` (`freerouting -de board.dsn -do board.ses`), then File > Import > Specctra Session (.ses). Expect to clean up the result by hand; the regular matrix is usually nicer hand-routed.
