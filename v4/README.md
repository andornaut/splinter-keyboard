...
`npm run ergogen` adds copper keepout rule areas to the generated boards. They ride the `cp` steps into `unrouted/` and `routed/` automatically; do not hand-edit them.

* A 2mm ring just inside the board edge (sized to clear the case support-wall lip, bridged straight across the USB cutout) and a 1mm-margin disk around each screw boss.
* The perimeter is two rings: a **pour ring** that keeps the GND plane off the whole edge, and a **route ring** that makes any track, via, or conductive pad a DRC error, except a top-edge band above the TRRS (carved out, since the case's top wall has the port opening there, so the TRRS top through-holes reach the edge). The route ring keeps the inner vertical edge full height, so copper under the side wall is still flagged. The boss disks exclude pour, tracks, and vias.
* Bare mechanical (NPTH) holes (the mounting holes and the TRRS locating hole, which carry no copper) are always allowed. The conductive-pad check is a custom DRC rule in each board's `<name>.kicad_dru` file (it keys on pad type, so SMD lands like the hotswap sockets are caught too).
