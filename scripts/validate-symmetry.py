...
def edge_segments(pcb_path):
    ...
    with pcb_path.open(encoding="utf-8") as f:
        text = f.read()
...
def check_components(a_path, b_path):
    ...
    for item, half in [(i, a_path) for i in only_a] + [(i, b_path) for i in only_b]:
        if item["lib"] not in KEY_FOOTPRINTS:
            fails.append(
                f"{item['ref']} ({item['lib']}) on {half.name}: "
    ...
    for half, unpaired in ((a_path, only_a), (b_path, only_b)):
        ...
        if expected is None:
            fails.append(f"{half.name}: no expected unpaired-key count is recorded for this half in UNPAIRED_KEYS")
        elif len(key_unpaired) != expected:
            fails.append(
                f"{half.name}: {len(key_unpaired)} unpaired key "
    ...
    for z, half in [(i, a_path) for i in zone_a] + [(i, b_path) for i in zone_b]:
        fails.append(f"zone {z['name']} on {z['layer']} of {half.name}: no counterpart at the mirrored position")
    ...
    for g, half in [(i, a_path) for i in gfx_a] + [(i, b_path) for i in gfx_b]:
        ...
        if g["layer"] != KEY_REFERENCE_LAYER:
            fails.append(
                f"{g['kind']} on {g['layer']} of {half.name} at "
