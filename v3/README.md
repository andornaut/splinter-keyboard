...
1. Run `npm run build` to generate and save outlines and pcbs to `../dist/v2/ergogen/` then `npm run copy-pcbs-to-kicad`
...
1. Route the PCBs in [`./kicad/`](./kicad/), and then save them to [`./kicad/routed/`](./kicad/routed/)
    * Once you're happy with the routing, run `npm run copy-pcbs-to-routed` to copy traces to [`./kicad/routed/`](./kicad/routed/)
    * If you've generated new PCB files using Ergogen, then you can run `npm run copy-traces-from-routed` to copy traces from the PCBs in [`./kicad/routed/`](./kicad/routed/) back to those of the same name in [`./kicad/`](./kicad/). Select File > Revert > Yes to refresh the pcb in Kicad.
